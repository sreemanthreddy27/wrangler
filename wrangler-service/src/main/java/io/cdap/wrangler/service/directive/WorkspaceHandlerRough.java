...
//import io.cdap.wrangler.api.NullHandlingException;
...
//          GSON.fromJson(StandardCharsets.UTF_8.decode(
//          request.getContent()).toString(), WorkspaceCreationRequest.class);
//
...
//      Workspace workspace = Workspace.builder(generateWorkspaceName(wsId,
...
//  }
//
//  public List<Row> nullChecker(List<Row> sample, HashMap<String, UserDefinedAction> nullabilityMap)
//      throws NullHandlingException {
...
//
//      for (Entry<String, UserDefinedAction> columnNullability : nullabilityMap.entrySet()) {
//        String columnName = columnNullability.getKey();
//        UserDefinedAction userDefinedAction = columnNullability.getValue();
...
//              break;
//            case ERROR_PIPELINE:
//              throw new NullHandlingException(String.format(
//              "null value found in non-nullable column %s", columnName));
//          }
//        } else {
//          noActionCount++;
//        }
//      }
//      if (nullPresentinRow) {
//        if (noActionCount == nullabilityMap.size()) {
//          result.add(row);
//        }
//      } else {
//        result.add(row);
...
//        columnMappings = null;
//      }
//      String columnMappingsString = columnMappings != null ? columnMappings.toString() : "latitude=SKIP_ROW";
//
...
////          "nonNullableColumns", nonNullableColumnsString,
////          "userDefinedAction", userDefinedAction.toString()
...
//    UserDirectivesCollector userDirectivesCollector = new UserDirectivesCollector();
//    boolean useNullCheckedRows = executionRequest.getNullabilityMap() != null;
//    List<Row> nullCheckedRows = new ArrayList<>();
//    if (useNullCheckedRows) {
//      HashMap<String, UserDefinedAction> nullabilityMap = executionRequest.getNullabilityMap();
//      //part 1 change nullabilityColumnMap in Workspace Object
//      changeNullability(nullabilityMap, workspaceId);
//
//      //part 2 go through all the rows to check for null in non-nullable columns
//      nullCheckedRows = nullChecker(detail.getSample(), nullabilityMap);
//    }
//
//    List<Row> result = executeDirectives(ns.getName(), directives, detail,
//        userDirectivesCollector, nullCheckedRows, useNullCheckedRows);
...
//    } catch (Exception e) {
//      throw new RuntimeException("Error in setting nullability of the columns ", e);
//    }
//  }
//
//
...
//        grammarVisitor, inputRows, useInputRows, detail.getWorkspace());
//  }
//
//  /**
...
//    Row row = sample.get(i);
//    int elsecounts = 0;
//    boolean nullPresentinRow = false;
//
//    Iterator columnMappingsIterator = nullabilityMap.entrySet().iterator();
//
//    while (columnMappingsIterator.hasNext()) {
//    HashMap.Entry mapElement
//    = (HashMap.Entry) columnMappingsIterator.next();
...
//    }
//    if (nullPresentinRow) {
//    if (elsecounts == nullabilityMap.size()) {
//    result.add(row);
//    }
//    } else {
//    result.add(row);
